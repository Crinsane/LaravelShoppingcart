...
class Cart {
	 ...
	 */
	public function add($id, $name = null, $qty = null, $price = null, $image = null, array $options = [])
	{
		...
		if(is_array($id))
		{
			...
			if($this->is_multi($id))
			{
				...
				foreach($id as $item)
				{
					...
					$this->addRow($item['id'], $item['name'], $item['qty'], $item['price'], $item['image'], $options);
				}
			...
			$result = $this->addRow($id['id'], $id['name'], $id['qty'], $id['price'], $id['image'], $options);
		...
		// Fire the cart.add event
		$this->event->fire('cart.add', compact('id', 'name', 'qty', 'price', 'image', 'options'));

		$result = $this->addRow($id, $name, $qty, $price, $options);

		// Fire the cart.added event
		$this->event->fire('cart.added', compact('id', 'name', 'qty', 'price', 'image', 'options'));
	 ...
	 * @param array   $options  Array of additional options, such as 'size' or 'color'
	 */
	protected function addRow($id, $name, $qty, $price, $image, array $options = [])
	{
		if(empty($id) || empty($name) || empty($qty) || empty($image) || ! isset($price))
		{
		...
		{
			$cart = $this->createRow($rowId, $id, $name, $qty, $price, $image, $options);
		}
	 ...
	 * @return Gloudemans\Shoppingcart\CartCollection
	 */
	protected function createRow($rowId, $id, $name, $qty, $price, $image, $options)
	{
		$cart = $this->getContent();
